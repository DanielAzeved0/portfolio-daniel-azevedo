...
export default function CertificateCarousel({ certifications }: CertificateCarouselProps) {
  ...
  useEffect(() => {
    ...
    const updateActive = () => {
      ...
      const closest = slides.reduce((best, slide, i) => {
        ...
        return Math.abs(sc - center) <
          Math.abs(slides[best].offsetLeft + slides[best].offsetWidth / 2 - center)
          ? i
          : best;
      }, 0);
  ...
  const handlePointerDown = (e: PointerEvent<HTMLDivElement>) => {
    ...
    try {
      track.setPointerCapture((e as any).pointerId);
    } catch {}
  };
    ...
    <div className="relative overflow-visible">
      <div className="mb-6 flex items-center justify-between gap-4">
        ...
        <div className="flex gap-3">
          <button
            ...
            className={`grid h-11 w-11 place-items-center rounded-full border bg-white disabled:cursor-not-allowed disabled:opacity-45 ${FOCUS_STYLES}`}
          >
            ◀
          </button>
          <button
      ...
      >
        {certifications.map((cert, index) => {
          ...
          const slideLabel =
            "Slide " + (index + 1) + " de " + certifications.length + ": " + cert.title;
          const slideClasses =
            "w-[86%] flex-none snap-center sm:w-[58%] lg:w-[38%] " +
              ...
              <article className="flex h-full min-h-64 flex-col justify-between bg-white p-6 rounded-xl border-2">
                <div>
                  <div className="mb-5 flex items-start justify-between gap-4">
                    <div className="grid h-14 w-14 place-items-center rounded-xl bg-[var(--background)] text-3xl">
                      {cert.icon}
                    </div>
                ...
                <div>
                  <p className="mb-1 font-semibold text-[var(--accent-primary)]">
                    {cert.institution}
                  </p>
                  {cert.date && (
                  ...
                  {cert.certificateUrl && (
                    <a
                      ...
                      className={`mt-5 inline-flex items-center gap-2 rounded-sm text-sm font-bold text-[var(--accent-secondary)] ${FOCUS_STYLES}`}
                    >
                      Ver certificado
                    </a>
                  )}
      ...
      <div className="flex justify-center gap-2">
        {certifications.map((cert, index) => (
          <button
            ...
            className={`h-3 rounded-full transition-all ${FOCUS_STYLES} ${activeIndex === index ? "w-9 bg-[var(--accent-secondary)]" : "w-3 bg-[rgba(51,51,51,0.25)]"}`}
          />
        ))}
      </div>
    </div>
