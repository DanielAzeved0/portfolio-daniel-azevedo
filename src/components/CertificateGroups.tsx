...
export default function CertificateGroups({ certifications }: CertificateGroupsProps) {
  const categories = Array.from(new Set(certifications.map((cert) => cert.category ?? "Outros")));
  const [activeCategory, setActiveCategory] = useState(categories[0] ?? "Outros");
  const activeCertifications = certifications.filter((cert) => cert.category === activeCategory);
  ...
  return (
    <div className="space-y-8">
      <div
        ...
        role="tablist"
        aria-label="Categorias de certificados"
      >
      ...
      >
        {activeCertifications.length > 0 ? (
          <CertificateCarousel key={activeCategory} certifications={activeCertifications} />
        ) : (
